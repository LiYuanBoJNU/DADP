...
class LitModel(pl.LightningModule):
    ...
    def render(self, noise, cond=None, T=None, inter=False, t=None, is_last=False):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
        ...
        if cond is not None:
            if inter:
                out = render_condition_inter(self.conf,
                                            self.ema_model,
                                            noise,
                                            sampler=sampler,
                                            cond=cond,
                ...
                if is_last:
                    pred_img = out["sample"]
                    pred_img = (pred_img + 1) / 2
                    return pred_img, pred_img
                else:
                    return pred_img, out["sample"]
            else:
    ...
    def encode_stochastic_all(self, x, cond, T=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
    ...
    def encode_stochastic_t(self, x, cond, T=None, t=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()

        out = sampler.ddim_reverse_sample(self.ema_model,
                                       x,
                                       t=t,
                                       model_kwargs={'cond': cond})
        return out['sample']

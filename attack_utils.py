...
class Net_my(torch.nn.Module):
    def __init__(self, test_models, decoder=None, cls_model=None):
        super(Net_my, self).__init__()
        self.test_models = test_models
        self.decoder = decoder
    ...
    def forward(self, z, xT=None, T=1, t=None, predict=True, is_last=False, inter=True):
        ...
        if xT is None:  # input is images
            ...
            # x = self.decoder.render(xT, z, T)
            # x = self.decoder.render(z, xT, T)
            x, x_pred = self.decoder.render(z, xT, T, inter, t, is_last)
            # print(x.size())
            # x = out["pred_xstart"]
            # x_pred = out["sample"]
